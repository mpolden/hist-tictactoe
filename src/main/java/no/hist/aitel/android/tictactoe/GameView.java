...
public class GameView extends View {
    ...
    public void setCell(int cellIndex, GameState value) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (isEnabled() && x >= 0 && x < boardSize && y >= 0 & y < boardSize) {
                ...
                GameState state = cell == selectedCell ? selectedValue : data[cell];
                state = state == GameState.EMPTY ? currentPlayer : GameState.EMPTY;
                //stopBlink();
                selectedCell = cell;
                selectedValue = state;
                if (data[selectedCell] == GameState.EMPTY) {
